        ...
        {done?(<Card style={{textAlign:"center",padding:24}}>
          ...
          <>
            ...
            <div style={{display:"flex",gap:8,marginBottom:16}}>
              {[{id:"pi",lbl:"π Pi",c:C.gold},{id:"sbc",lbl:"S SBC",c:C.light}].map(m=>(<div key={m.id} onClick={()=>setMethod(m.id)} style={{flex:1,padding:"11px 0",textAlign:"center",borderRadius:12,border:`1px solid ${method===m.id?m.c:C.border}`,background:method===m.id?`${m.c}22`:C.card,cursor:"pointer",fontWeight:800,color:m.c}}>{m.lbl}</div>))}
            </div>
