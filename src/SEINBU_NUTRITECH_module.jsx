      ...
      {tab==="subs"&&<div style={{padding:16}}>
        ...
        {SUBS.map(s=>(<Card key={s.id} style={{background:s.pop?`${s.color}22`:C.card,border:`1px solid ${s.pop?s.color:C.border}`}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            ...
            <div style={{textAlign:"right"}}><div style={{fontSize:16,fontWeight:900,color:C.gold}}>{fmt(s.price)} FCFA</div><div style={{fontSize:8,color:C.sub}}>{toPi(s.price)} π</div><div style={{fontSize:9,color:"#4ADE60"}}>{fmt(s.price/10,0)} SBC{t.monthly}</div></div>
          </div>
